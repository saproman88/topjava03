...
/* My work
       Map<LocalDate, Integer> groups=mealList.stream().collect(groupingBy(m -> (m.getDateTime().toLocalDate()),summingInt(UserMeal::getCalories)));
        ...
        return mealList.stream().filter(m->(m.getDateTime().toLocalTime().isAfter(startTime)&&m.getDateTime().toLocalTime().isBefore(endTime))).map(m->new UserMealWithExceed(m.getDateTime(),m.getDescription(),m.getCalories(),groups.get(m.getDateTime().toLocalDate())>caloriesPerDay)).collect(Collectors.toList());
*/
    }
}
